...
distribution~\cite{Fraley00model-basedclustering}:
    ...
    acquisition.% TODO: add ref. et example
...
dataset and  the second,  called \emph{Potsdam},  is an  airborne very
high resolution multispectral image.
    ...
    \begin{figure}[!t]  %TODO: elle  est  pourrie cette  colorisation!
      % Prend trois bandes dans le visible (RGB) et
      % étire  correctement l'histogramme. De  plus, on ne sait  pas à
      %  quelle  couleur correspond  la  couleur.  Tu devrais  pouvoir
      % récupérer  cela depuis QGIS.  Il faudrait le  rajouter quelque
      % part. J'ai du code TIKZ pour le faire ci-besoin ...
        \centering
        \begin{tabular}{c}
...
\label{sec:test}
    ...
    \begin{itemize}
        ...
        \item A Random Forest classifier with parameters optimized by gridsearch (200 trees, 40 max depth, 50 size of subset of variables)%%TODO : pas clair ce qu'est "50 size of subset of variables"
    ...
    %% TODO : c'est encore le cas ? à verifier: si on prends quand cela arréte de croitre rapidement il faut changer
    For SFS and  SFFS selection, the number of variables  to select is
    set to 30 for  the Aisa datset and 60 for  the Potsdam dataset and
    then  the  number  of  variables  which  maximizes  the  criterion
    ...
    %% TODO; Reprendre cette partie: ne parle pas à la fois de la
    %% classification et du temps de calcul. D'abors du commences à
    %% décrire et commenter sur la classification. Ensuite tu fais la
    %% même chose sur les temps de calculs. Et il faut en dire plus !!
    %%   Pour   les   classifs,   il  faut   parler   des   tests   de
    %%  significativité.  Pour les  temps  de  calculs, il  faut  plus
    %% détailler  pourquoi cela  va plus  ou moins  vite. Avec  FS, on
    %% réduit le nombre de variables et comme la régle de décision est
    %%  de compléxité  (d^3)  donc  quand on  réduit  d, on  améliorer
    %% grandement  les temps de  calcul, pour les RF,  plus il y  a de
    %% training samples, plus lse arbres vont êtres grands ...
    The  results show  that,  on this  dataset,  GMM classifiers  with
    feature selection  get the  best classification  rate.  Especially
    with  very  few training  samples,  GMM  classifiers with  feature
    selection outperform  the others.  In term  of computational time,
    the GMM classifiers  are as expected very  fast for classification
    and also  for training,  except when the  criterion function  is a
    classification rate.  Among the  three variations of the selection
    algorithm, none appears  to perform better than  the others. Using
    kappa  or Jeffries-Matusita  distance  as criterion  is equal  and
    using SFFS  does not give any  advantage. In this case,  using J-M
    distance  as criterion  and SFS  as  search strategy  is the  best
    choice in term of time efficiency.  % The computation of the kappa
                                       % seems  to suffer from  a lack
                                       %   of  parallelization   which
                                       %  could be  improved. ->  plus
                                       % tard dans la conclusion.

    Nevertheless,  good  performance  are also  obtained  with  Random
    Forest which becomes  more competitive when the  number of samples
    increases.  The KNN  classifier and the GMM  classifier with ridge
    regularization   are   totally   outperform  both   in   term   of
    classification rate and of processing time.
    ...
    \begin{table}[!t]
        \centering
        \caption{Kappa coefficient and processing time for 1000 samples by class and averaged over 5 trials (standard deviation in parenthesis). Processing times are given in second.\label{tab:potsdam-otbsimu}}
        \begin{tabular}{lccccc}\toprule
          & {\bfseries 5\_11 (train)} & {\bfseries 5\_12 (test)} & {\bfseries Train. Time} & {\bfseries Classif. time} & {\bfseries \# of selected features} \\ \cline{2-6}
    ...
    %% TODO: je te laisse modifier le tableau de la même manière (style, legende etc ...)
    \begin{table}[!t]
        \centering
      ...
      %% TODO: reprendre la discussion avec les commentaires de l'image précédente
